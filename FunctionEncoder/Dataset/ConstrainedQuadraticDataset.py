...
class ConstrainedQuadraticDataset(BaseDataset):
    ...
    def sample(self) -> Tuple[  torch.tensor, 
        ...
        with torch.no_grad():
            ...
            As = torch.rand((n_functions, 1), dtype=torch.float32, device=self.device) * (self.a_range[1] - self.a_range[0]) + self.a_range[0]
            Bs = torch.rand((n_functions, 1), dtype=torch.float32, device=self.device) * (self.b_range[1] - self.b_range[0]) + self.b_range[0]
            Cs = torch.rand((n_functions, 1), dtype=torch.float32, device=self.device) * (self.c_range[1] - self.c_range[0]) + self.c_range[0]

            # generate n_samples_per_function samples for each function
            xs = torch.rand((n_functions, n_points, *self.input_size), dtype=torch.float32, device=self.device)
            xs = xs * (self.input_range[1] - self.input_range[0]) + self.input_range[0]
            example_xs = torch.rand((n_functions, n_examples, *self.input_size), dtype=torch.float32, device=self.device)
